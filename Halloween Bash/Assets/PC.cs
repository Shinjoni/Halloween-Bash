...
public class PC : MonoBehaviour {
	...
	public AudioClip woahSfx;
	public AudioClip woah2Sfx;
	public AudioClip clownSfx;
	...
	void OnTriggerEnter2D(Collider2D col) {
		...
		if (col.CompareTag ("woah")) {
			GetComponent<AudioSource> ().PlayOneShot (woahSfx, 1.0f);
		}
			
	if (col.CompareTag ("woah2")) {
		GetComponent<AudioSource> ().PlayOneShot (woah2Sfx, 1.0f);
	}

		if (col.CompareTag ("popoutclown")) {
			popoutclown.SetActive (true);
			GetComponent<AudioSource> ().PlayOneShot (clownSfx, 1.0f);
